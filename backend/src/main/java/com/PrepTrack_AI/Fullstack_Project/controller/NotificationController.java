...
public class NotificationController {
    ...
    public ResponseEntity<ApiResponse<List<NotificationResponseDTO>>> getUnreadNotifications(Principal principal) {
        ApiResponse<PagedResponse<NotificationResponseDTO>> res = notificationService.getUserNotifications(principal.getName(), 0, 100);
        List<NotificationResponseDTO> unread = res.getData() != null && res.getData().getContent() != null
                ? res.getData().getContent().stream()
